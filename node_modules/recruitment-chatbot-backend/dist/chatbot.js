...
function createChatRouter() {
    ...
    router.post('/messages', async (req, res, next) => {
        try {
            ...
            const senderType = authReq.user?.role === 'hr' ? 'hr' : 'candidate';
            const { sessionId: providedSessionId, content } = req.body;
            ...
            const botReplyText = await handleChatMessage(content, senderType, session.id);
            const botMsg = {
                id: generateId('msg'),
                sessionId: session.id,
                senderType: 'bot',
    ...
    router.post('/cv', cvUpload.single('cv'), async (req, res, next) => {
        try {
            const authReq = req;
            const userId = authReq.user?.id || 'anonymous';
            const senderType = authReq.user?.role === 'hr' ? 'hr' : 'candidate';
            ...
            const jobTitleRaw = req.body?.jobTitle?.trim();
            const session = ensureSession(userId, providedSessionId);
            const selectedJob = jobTitleRaw === 'Senior Full-Stack Developer'
                ...
                : jobTitleRaw === 'Talent Acquisition Lead (HR)'
                    ...
                    : jobTitleRaw === 'Senior Financial Analyst'
                        ...
                        : null;
            if (!selectedJob) {
                const botReplyText = 'Please select a job first, then upload your CV (PDF) for screening.';
                const botMsg = {
                    id: generateId('msg'),
                    sessionId: session.id,
                    senderType: 'bot',
            ...
            if (!extractedText.trim()) {
                const botReplyText = '❌ Could not extract text from this PDF. Please upload a text-based PDF (not a scanned image).';
                const botMsg = {
                    id: generateId('msg'),
                    sessionId: session.id,
                    senderType: 'bot',
            ...
            if (e.approved) {
                ...
                (0, approvedCvsStore_1.addApprovedCv)({
                    id: generateId('cv'),
                    createdAt: new Date().toISOString(),
                    ...
                    pdfSha256: (0, approvedCvsStore_1.sha256Pdf)(file.buffer),
                });
                const candidateEmail = extractFirstEmail(extractedText);
                if (candidateEmail) {
                    try {
                        const sent = await (0, email_1.sendApprovedEmail)({
                            to: candidateEmail,
                            jobTitle: selectedJob,
                        });
                        emailNote = sent.sent
            ...
            const suggestionNote = !e.approved && approvedAlternatives.length > 0
                ...
                : '';
            const botReplyText = '✅ CV scanned. Here is the result (automated screening):\n\n' +
                (() => {
                    const years = e.yearsDetected === null ? 'not detected' : `${e.yearsDetected}`;
                    if (e.approved) {
                        return (`✅ Status: APPROVED — ${e.jobTitle}\n` +
                    ...
                    return (`❌ Status: REJECTED — ${e.jobTitle}\n` +
                        `- Experience: ${years} years\n` +
...
function ensureSession(userId, sessionId) {
...
const JOB_BUTTONS_LINE = '[JOB_BUTTONS:Senior Full-Stack Developer|Talent Acquisition Lead (HR)|Senior Financial Analyst]';
...
function buildGrokMessages(sessionId) {
    const list = messages.get(sessionId) || [];
...
async function handleChatMessage(content, senderType, sessionId) {
    ...
    if ((0, aiChat_1.isAiChatConfigured)()) {
        try {
            return await (0, aiChat_1.aiChat)(buildGrokMessages(sessionId));
        }
        catch (err) {
    ...
    if (senderType === 'candidate') {
        const jobTitle = normalized === 'senior full-stack developer'
            ? 'Senior Full-Stack Developer'
        ...
        if (jobTitle) {
            if (jobTitle === 'Senior Full-Stack Developer') {
                return ('Requirements for Senior Full-Stack Developer:\n' +
                    '- Mandatory: Experience >= 5 years\n' +
                    '- Mandatory keywords: Python AND React\n' +
                    '- Bonus: +10 AWS, +10 Docker\n');
            }
            if (jobTitle === 'Talent Acquisition Lead (HR)') {
                return ('Requirements for Talent Acquisition Lead (HR):\n' +
                    '- Mandatory: Experience >= 4 years\n' +
                    '- Mandatory keywords: SHRM OR PHRi\n' +
                    '- Bonus: +10 Payroll, +10 LinkedIn Recruiter\n');
            }
            return ('Requirements for Senior Financial Analyst:\n' +
                '- Mandatory: Experience >= 8 years\n' +
                '- Mandatory keywords: CMA OR CFA\n' +
                '- Bonus: +15 Oracle OR SAP\n');
        }
    ...
    if (text.includes('job') || text.includes('position') || text.includes('role')) {
        return ('Open roles we support right now:\n\n' +
            '• Senior Full-Stack Developer\n' +
            '• Talent Acquisition Lead (HR)\n' +
            '• Senior Financial Analyst\n\n' +
    ...
    if (senderType === 'candidate' && (text.includes('hi') || text.includes('hello'))) {
        return ('Hi! These are the 3 roles we screen for (same as the AI Recruitment Portal / Streamlit flow):\n\n' +
    ...
    return ('I can help with our three open roles and CV upload, but natural AI chat needs an API key (free options available).\n\n' +
