...
function envProvider(): Provider | null {
  ...
  if (p === 'gemini' || p === 'groq' || p === 'xai') return p;
  return null;
}
...
function pickProvider(): Provider | null {
  ...
  if (forced === 'xai' && process.env.XAI_API_KEY?.trim()) return 'xai';
...
async function geminiChat(messages: AiChatMessage[]): Promise<string> {
  ...
  if (!text.trim()) {
    throw new Error(
      data.error?.message || 'Empty Gemini response (blocked or no candidates)',
    );
  }
  return text.trim();
}

async function groqChat(messages: AiChatMessage[]): Promise<string> {
